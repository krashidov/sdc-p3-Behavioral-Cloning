...
# The first layer will turn 1 channel into 32 channels, stride of 2x2
model.add(Convolution2D(32, 5, 5, border_mode='valid', subsample=(2,2), batch_input_shape=(90, 320, 16, 3)))
model.add(Activation('relu'))

# The second conv layer will convert 32 channels into 16 channels, stride of 2x2
model.add(Convolution2D(16, 5, 5, subsample=(2,2)))
model.add(Activation('relu'))

# The third conv layer will convert 16 channels into 8 channels
model.add(Convolution2D(8, 5, 5))
model.add(Activation('relu'))

# The fourth conv layer will convert 8 channels into 4 channels
model.add(Convolution2D(4, 3, 3))
model.add(Activation('relu'))

#The fifth conv layer will convert 4 channels into 2 channels
model.add(Convolution2D(2, 3, 3))
model.add(Activation('relu'))

# Apply Max Pooling with 2x2 pool size
...
# Input 16 Output 1 steering angle
model.add(Dense(1))

#Adam optimizer with Mean Squared Error
